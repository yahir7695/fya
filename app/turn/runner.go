...
type Catalog interface {
	Select(cwd string, since time.Time, prompt string) (transcript.Candidate, error)
}
...
func (r *Runner) Run(ctx context.Context, cfg Config) error {
	...
	if typeErr := r.inject.Type(ctx, stringWriter{session: session}, cfg.Prompt); typeErr != nil {
		return fmt.Errorf("type prompt: %w", typeErr)
	}

	candidate, err := r.selectTranscript(ctx, cfg, session.Done())
	if err != nil {
	...
	return r.streamTranscript(ctx, cfg, session, r.tailers(candidate.Path))
}
...
func (r *Runner) selectTranscript(ctx context.Context, cfg Config, sessionDone <-chan struct{}) (transcript.Candidate, error) {
	ticker := time.NewTicker(cfg.PollPeriod)
	defer ticker.Stop()
	for {
		candidate, err := r.catalog.Select(cfg.CWD, cfg.StartedAt, cfg.Prompt)
		if err == nil {
			return candidate, nil
		}
		if !errors.Is(err, transcript.ErrNoTranscript) {
			return transcript.Candidate{}, fmt.Errorf("select transcript: %w", err)
		}
		select {
		case <-ctx.Done():
			return transcript.Candidate{}, fmt.Errorf("select transcript: %w", ctx.Err())
		case <-sessionDone:
			return transcript.Candidate{}, errors.New("select transcript: claude exited before transcript was written")
		case <-ticker.C:
		}
	}
}
...
// stringWriter adapts Session.WriteString to io.Writer so the Injector can use
// the writer interface without a separate string-writing method.
type stringWriter struct {
	session Session
}

func (w stringWriter) Write(p []byte) (int, error) {
	if err := w.session.WriteString(string(p)); err != nil {
		return 0, fmt.Errorf("write session string: %w", err)
	}
	return len(p), nil
}
...
// NewTailerFactory returns the production TailerFactory backed by transcript.NewTailer.
func NewTailerFactory() TailerFactory {
	return func(path string) Tailer {
		return transcript.NewTailer(path, 0)
	}
}
